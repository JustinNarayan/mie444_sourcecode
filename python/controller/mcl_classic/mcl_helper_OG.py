...
with open("C:/Users/Clair/OneDrive/Documents/MIE444/sensor_data_12_ppi_15_beam_angle.pkl",'rb') as f:
...
# In mcl_helper_OG.py, inside the Rover class:
    
    # NOTE: You should remove the old 'move(self, dt, grid)' method if you no longer use it.
    
    def move_delta(self, delta_x, delta_y, delta_theta, grid):
        """Move the rover by a commanded delta (dx, dy, dtheta), with noise.
        This assumes the command is in the world frame, simplifying the model for testing."""
        
        # Apply noise to movement model
        dx_noise = random.gauss(0, MOVEMENT_NOISE)
        dy_noise = random.gauss(0, MOVEMENT_NOISE)
        angular_noise = random.gauss(0, math.radians(2))
        
        # Calculate the actual (noisy) movement
        actual_dx = delta_x + dx_noise
        actual_dy = delta_y + dy_noise
        actual_dtheta = delta_theta + angular_noise
        
        # Calculate proposed new position (Direct addition in world coordinates)
        new_x = self.x + actual_dx
        new_y = self.y + actual_dy
        new_theta = self.theta + actual_dtheta

        # Check if new position is within drivable area
        if 0 <= int(new_x) < len(grid[0]) and 0 <= int(new_y) < len(grid) and grid[int(new_y)][int(new_x)] == 0:
            self.x = new_x
            self.y = new_y
        self.theta = new_theta
...
class Particle:
    ...
    def move(self, delta_x_command, delta_y_command, delta_theta_command, grid):
        ...
        dx_noise = random.gauss(0, MOVEMENT_NOISE)
        dy_noise = random.gauss(0, MOVEMENT_NOISE)
        angular_noise = random.gauss(0, math.radians(2)) # Using the same noise parameter
