...
def main():
    ...
    try:
        while not stop_event.is_set():
            # Drivetrain
            drivetrain_key_pressed = None
            for k in VALID_KEYS:
                if keyboard.is_pressed(k):
                    drivetrain_key_pressed = k
                    break  # Only take the first pressed key

            if drivetrain_key_pressed:
                msg = encode_message(MessageType.DrivetrainManualCommand, drivetrain_key_pressed)
                ser.write(msg)
                ser.flush()
            
            # Lidar
            lidar_key_pressed = None
            for k in LIDAR_KEYS:
                if keyboard.is_pressed(k):
                    lidar_key_pressed = k
                    break
